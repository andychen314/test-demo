 ...
 * @date 2016/8/3.
 */
public class DefaultEventController implements IEventController {
    ...
    private IMsgSend eventTemplate; // 给App使用的Event发送客户端

    private AtomicBoolean isStarted = new AtomicBoolean(false);

    private static DefaultEventController defaultEventController;

    public synchronized static DefaultEventController getInstance(RabbitMQServerConfig config) {
    ...
    private DefaultEventController(RabbitMQServerConfig config) {
        if (config == null) {
        ...
        eventTemplate = new DefaultMsgSend(rabbitTemplate, defaultCodecFactory);
    }
    ...
    @Override
    public IMsgSend getEopEventTemplate() {
        return eventTemplate;
    }

    @Override
    public IEventController add(String queueName, String exchangeName, EventProcesser eventProcesser) {
        return add(queueName, exchangeName, eventProcesser, defaultCodecFactory);
    }

    public IEventController add(String queueName, String exchangeName, EventProcesser eventProcesser, ICodecFactory codecFactory) {
    ...
    @Override
    public IEventController add(Map<String, String> bindings, EventProcesser eventProcesser) {
        return add(bindings, eventProcesser, defaultCodecFactory);
    }

    public IEventController add(Map<String, String> bindings, EventProcesser eventProcesser, ICodecFactory codecFactory) {
